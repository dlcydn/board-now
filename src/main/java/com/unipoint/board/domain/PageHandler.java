...
//    private String  keyword;
    ...
    private int totalCnt; // 게시물의 총 갯수 //count of all posting
    private int totalPage; // 전체 페이지의 갯수 //count of all pages
    private int beginPage; // 화면에 보여줄 첫 페이지 //the first page
    private int endPage; // 화면에 보여줄 마지막 페이지 //the last page
    private boolean showNext = false; // 이후를 보여줄지의 여부. endPage==totalPage이면, showNext는 false
    // boolean that wanna show the next - if endPage == totalPage, showNext is false
    private boolean showPrev = false; // 이전을 보여줄지의 여부. beginPage==1이 아니면 showPrev는 false
    // boolean that wanna show the prev - if beginPage == 1, showPrev is false
    ...
    private void doPaging(int totalCnt, SearchCondition sc) {
        ...
        this.sc.setPage(Math.min(sc.getPage(), totalPage));  // page가 totalPage보다 크지 않게 //page can not bigger than total Page
        this.beginPage = (this.sc.getPage() -1) / NAV_SIZE * NAV_SIZE + 1; // 11 -> 11, 10 -> 1, 15->11. separately test
